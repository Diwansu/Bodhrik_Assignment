...
def trigger_evaluation(
    ...
    evaluation = (
        ...
        .filter(models.Evaluation.session_id == session_id)
        .first()
    )
    ...
    if evaluation:
        ...
        evaluation = models.Evaluation(
            session_id=session_id, status=models.EvaluationStatus.PENDING.value
        )
        db.add(evaluation)
    ...
    try:
        ...
        logger.error(
            f"Failed to enqueue Celery task for evaluation {evaluation.id}: {e}"
        )
    ...
    try:
        ...
        logger.warning(
            f"Failed to invalidate cache for session {session_id}: {cache_err}"
        )
