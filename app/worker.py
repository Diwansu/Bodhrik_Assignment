...
celery = Celery("bodhrik_tasks", broker=settings.REDIS_URL, backend=settings.REDIS_URL)
...
def run_evaluation_job(evaluation_id: int) -> dict:
    ...
    try:
        ...
        logger.info(
            f"Evaluation {evaluation_id} completed successfully with score: {evaluation.score}"
        )
        ...
        try:
            ...
            logger.warning(
                f"Failed to invalidate cache for session {evaluation.session_id}: {cache_err}"
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to rollback and save error status for evaluation {evaluation_id}: {rollback_err}"
            )
